...
MetaManager::MetaManager() {
  ...
  auto json_config = read_config_from_file();
  auto local_node = json_config.get("local_compute_node");
  local_machine_id = (node_id_t)local_node.get("machine_id").get_int64();
  auto server_nodes = json_config.get("server_nodes");
  auto remote_ips = server_nodes.get("ips");
  auto remote_ports = server_nodes.get("ports");
  // txn_system = local_node.get("txn_system").get_int64();

  // auto cto_node = json_config.get("cto_node");
  // auto cto_ip = cto_node.get("cto_ip");
  // auto cto_port = cto_node.get("cto_port");
  // cto = RemoteNode{.node_id = 100, .ip = cto_ip, .port = cto_port};
