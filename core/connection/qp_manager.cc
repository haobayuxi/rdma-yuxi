...
//     meta_man->GetRemoteHashMR(remote_node.node_id); ` remote_log_mr =
...
//     MemoryAttr local_mr =
//     meta_man->global_rdma_ctrl->get_local_mr(CLIENT_MR_ID); RCQP* data_qp =
//     meta_man->global_rdma_ctrl->create_rc_qp(create_rc_idx(remote_node.node_id,
//     (int)global_tid * 2),
...
//       if (rc == SUCC) {
//         data_qp->bind_remote_mr(remote_hash_mr);  // Bind the hash mr as the
//         default remote mr for convenient parameter passing
...
//       if (rc == SUCC) {
//         log_qp->bind_remote_mr(remote_log_mr);  // Bind the log mr as the
//         default remote mr for convenient parameter passing
