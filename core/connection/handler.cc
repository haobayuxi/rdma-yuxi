...
//   handler->send_cq = ibv_create_cq(handler->context, max_depth, NULL, NULL,
//   0); CPEN(handler->send_cq); handler->recv_cq =
//   ibv_create_cq(handler->context, max_depth, NULL, NULL, 0);
...
//   ret = sendData1(handler->fd, handler->l_private_data,
//   sizeof(private_data)); ret = readUntil1(handler->fd,
//   handler->r_private_data, sizeof(private_data));
...
// // offset) { 	struct ibv_sge sge = { 		(uint64_t)handler->buf +
// offset,
...
//   // 	printf("%02x ", *(char *)(handler->buf+handler->write_offset +
//   i));
//   // }
//   // printf("\n");
//   if (ret != -1) {
...
//   while (1) {
//     volatile char *end =
//         (volatile char *)(handler->receive_buf + handler->have_read +
//         msg_size);
//     if (*end == '1') {
//       break;
//     }
